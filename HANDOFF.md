...
## 한 줄 요약
모바일 위젯 마지막 버그(아이폰 자동확대) 수정 + 교회 찾는 질문에 대한 답변 톤 개선(2차에 걸쳐 목사님 상담 중복 맺음말까지 제거). 둘 다 실기/로컬 검증 완료. 프롬프트는 **`rag.py`·`api/ask.ts` 양쪽 동일하게** 수정.
...
## 2. 프롬프트 톤 — 교회 찾는 질문 대응 (`d8770ba`→`8b05199` chatbot / `0a280f8`→`6062c36` website)
- 증상: "어떤 교회를 찾아야 하나" 류 질문에도 기계적으로 "담임 목사님과 상담" 맺음말이 붙어 교회를 찾는 사람에게 어색.
- 1차 조치(`generate_prompt`): ① 안내를 질문자 상황별 분기 — 기존 신자→담임 목사님 상담, 교회 탐색 중→**센트럴처치 설교 청취+예배 방문 권유**, 매 답변 기계적 첨부 금지. ② 프롬프트/시스템 메시지에 **"센트럴처치" 정체성 명시**([설교 내용]=센트럴처치 설교)해 방문 추천이 자연스럽게 나오도록.
- 2차 조치(`8b05199`/`6062c36`) — **중복 맺음말 제거**: 1차 후에도 방문 권유 + "담임 목사님 상담"이 **함께** 붙던 문제(탐색자는 아직 담임 목사님이 없는데 부적절). 두 안내를 **상호배타**로 명시하고, 탐색 중인 분께는 담임 목사님 상담을 **절대 언급 말도록** 강제.
- 로컬 검증: 교회 탐색 질문 2건→센트럴처치 방문만(목사님 상담 없음) / 기존 신자 질문→담임 목사님 상담 유지(회귀 없음).
