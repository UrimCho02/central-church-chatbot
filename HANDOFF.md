# 작업 핸드오프 — 2026-06-09 (iOS 입력창 자동확대 수정) ⭐ 최신

## 한 줄 요약
06-07 모바일 수정 후에도 **아이폰에서 입력창 포커스 시 화면이 확대돼 전송버튼이 가려지던** 문제 추가 수정 → **아이폰 실기 검증 완료(안 밀림).** 백엔드(이 레포) 변경 없음. 작업은 `central-church-website/chat-widget-wip`.

## 수정 (`b00c108`, central-church-website)
...
- 조치: `src/App.jsx` textarea 글씨 14px → **16px**(자동확대 차단) + `index.html` viewport 메타에 `interactive-widget=resizes-content`(Android 키보드 대응). 기존 `visualViewport` 높이 보정은 iOS 폴백으로 유지.
